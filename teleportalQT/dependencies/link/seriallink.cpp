    ...
    : AbstractLink("SerialLink", parent)
{
    ...
    connect(&_port, &QSerialPort::errorOccurred, this, [this](QSerialPort::SerialPortError error) {
        ...
        case QSerialPort::NoError:
            break;
        default:
            qDebug()<< "Error is critical ! Port need to be closed.";
            qDebug()<< "Error:" << error;
            finishConnection();
            break;
        }
...
bool SerialLink::setConfiguration(const LinkConfiguration& linkConfiguration)
{
    ...
    if (!linkConfiguration.isValid()) {
        qDebug()<< LinkConfiguration::errorToString(linkConfiguration.error());
        return false;
    }
...
bool SerialLink::startConnection()
{
    ...
    if (isOpen()) {
        qDebug()<< "Serial port will be restarted.";
        finishConnection();
    }

    if (!_port.open(QIODevice::ReadWrite)) {
        qDebug()<< "Fail to open serial port:" << _port.error();
        return false;
    }
...
bool SerialLink::setLowLatency()
{
    ...
    if (!handle) {
        qDebug() << "Failed to get serial handle from OS.";
        return false;
    }
    ...
    if (handle) {
        ...
        if (::GetCommTimeouts(handle, &currentCommTimeouts)) {
            ...
            if (!::SetCommTimeouts(handle, &currentCommTimeouts)) {
                qDebug() << "Failed to set read interval timeout.";
                qDebug()  << "System error:" << ::GetLastError();
            }
        } else {
            qDebug() << "Failed to get COMMTIMEOUTS struct from system.";
            qDebug() << "System error:" << ::GetLastError();
        }
    } else {
        qDebug()  << "Failed to get serial handle from OS.";
    ...
    qDebug() << "Register path:" << settingsSystemKey;
    ...
    if (!latencyTimerValue.isValid()) {
        qDebug() << "Variable does not exist, register path is wrong.";
        return false;
    }

    if (latencyTimerValue.toInt() == 1) {
        qDebug() << "Low latency timer is already in register.";
        return true;
    }

    if (!settings.isWritable()) {
        qDebug() << "Register is not writable.";
        return false;
    }
    ...
    if (settingsStatus != QSettings::NoError) {
        qDebug()  << "Failed to save LatencyTimer setting.";
        qDebug() << "Settings status:" << settingsStatus;
        return false;
    }
